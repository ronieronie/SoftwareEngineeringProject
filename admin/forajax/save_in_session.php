...
if(isset($_SESSION['cart'])){
    ...
    $check_available = 0;
    $check_available=check_duplicate_product($company_name, $product_name, $parts, $packing_size);
    $available_qty = 0;
    $check_the_qty=0;
    
    if($check_available==0){
        $available_qty=check_qty($company_name, $product_name, $parts, $packing_size, $link);

        if($available_qty>=$qty){
            $b = array("company_name"=>$company_name,"product_name"=>$product_name, "parts"=>$parts, "packing_size"=>$packing_size, "price"=>$price,"qty"=>$qty);
            array_push($_SESSION['cart'],$b);
        }
        else{
            echo "Enter Amount is not available";
        }
    }
    else{
        $av_qty = 0;
        $exist_qty = 0;
        $exist_qty = check_the_qty($company_name, $product_name, $parts, $packing_size);
    ...
    $available_qty = check_qty($company_name, $product_name, $parts, $packing_size, $link);
    if($available_qty>=$qty){
        $_SESSION['cart']=array(array("company_name"=>$company_name, "product_name"=>$product_name, "parts"=>$parts, "packing_size"=>$packing_size, "price"=>$price, "qty"=>$qty));
